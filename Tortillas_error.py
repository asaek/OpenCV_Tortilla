# 0.1.0 Revision de errores de tortillas
...
stored_image = cv2.imread('Tortilla1.PNG', 0) 
...
while True:
	...
	string = "Coinciencias = " + str(matches_found)
	...
	if matches_found > set_threshold:
		...
		# To input a text string cv2.putText is used
		#cv2.putText(image, string, orgin, font, fontScale, color, thickness)
		cv2.putText(capturing,'Error Detectado',(200,50), cv2.FONT_HERSHEY_COMPLEX, 1 ,(0,255,0), 2)
    
	# Display object detected using imshow built-in function
	cv2.imshow('Deteccion Tortillas', capturing)
